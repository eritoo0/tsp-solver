...
ALGORITHMS = {
    'tsp_algo': [
        {'name': 'POP_SIZE', 'label': 'Population Size', 'type': 'number', 'default': 50},
        {'name': 'NUM_GENERATIONS', 'label': 'Number of Generations', 'type': 'number', 'default': 500},
        {'name': 'MUTATION_RATE', 'label': 'Mutation Rate', 'type': 'number', 'default': 0.2},
    ],
    'ga': [
        {'name': 'POP_SIZE', 'label': 'Population Size', 'type': 'number', 'default': 50},
        {'name': 'NUM_GENERATIONS', 'label': 'Number of Generations', 'type': 'number', 'default': 500},
    ...
    'aco': [
        {'name': 'ANT_COUNT', 'label': 'Number of Ants', 'type': 'number', 'default': 50},
        {'name': 'NUM_ITERATIONS', 'label': 'Number of Iterations', 'type': 'number', 'default': 500},
        {'name': 'ALPHA', 'label': 'Alpha', 'type': 'number', 'default': 1.0},
        {'name': 'BETA', 'label': 'Beta', 'type': 'number', 'default': 3.0},
        {'name': 'EVAPORATION_RATE', 'label': 'Evaporation Rate', 'type': 'number', 'default': 0.1},
        {'name': 'Q', 'label': 'Q Value', 'type': 'number', 'default': 100.0},
    ],
    'hbc': [
        {'name': 'POP_SIZE', 'label': 'Population Size', 'type': 'number', 'default': 30},
        {'name': 'NUM_ITERATIONS', 'label': 'Number of Iterations', 'type': 'number', 'default': 200},
        {'name': 'NUM_BEES', 'label': 'Number of Bees', 'type': 'number', 'default': 20},
        {'name': 'EXPLORATION', 'label': 'Exploration Rate', 'type': 'number', 'default': 0.3},
        {'name': 'INTENSITY', 'label': 'Intensity', 'type': 'number', 'default': 5},
    ],
    'sa': [
        {'name': 'INITIAL_TEMPERATURE', 'label': 'Initial Temperature', 'type': 'number', 'default': 100.0},
        {'name': 'COOLING_RATE', 'label': 'Cooling Rate', 'type': 'number', 'default': 0.995},
        {'name': 'MAX_ITER', 'label': 'Max Iterations', 'type': 'number', 'default': 10000},
...
def solver_app(request):
    ...
    if request.method == 'POST':
        ...
        threading.Thread(
            target=run_job,
            args=(job_id, algo, filename, data),
            daemon=True
        ).start()
...
def select_algorithms(request):
    ...
    """
    return render(request, 'compare_app/select.html', {
...
def compare_async(request):
    ...
    """
    return render(request, 'compare_app/compare_async.html', {
        'algorithms': ALGORITHMS,
        'default_filename': 'berlin52_coords.txt',
    })
...
def compare_results(request):
    ...
    params_a = {p['name']: request.POST.get(f"{algo_a}_{p['name']}") for p in ALGORITHMS[algo_a]}
    params_b = {p['name']: request.POST.get(f"{algo_b}_{p['name']}") for p in ALGORITHMS[algo_b]}
    ...
    context = {
        'data': [
            {'name': algo_a, 'distance': result_a['distance'], 'time': result_a['time']},
            {'name': algo_b, 'distance': result_b['distance'], 'time': result_b['time']},
        ]
    }
