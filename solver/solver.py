...
def solve_tsp(filename, callback, **params):
    ...
    candidate_list = []
    for i in range(NUM_CITIES):
        neighbors = sorted([j for j in range(NUM_CITIES) if j != i],
    ...
    best = min(population, key=lambda t: total_distance(
        t, distance_matrix, NUM_CITIES))
    ...
    for gen in range(NUM_GENERATIONS):
        ...
        if stagnation_counter >= 30:
            ...
            if diversity > 0.20:
                ...
                top_k = sorted(population, key=lambda t: total_distance(
                    t, distance_matrix, NUM_CITIES))[:n_refine]
                ...
                population = [large_perturbation(
                    ind, NUM_CITIES, intensity=4) for ind in population]
        ...
        if stagnation_counter >= 60:
            population = aggressive_restart(
                population, NUM_CITIES, POP_SIZE, distance_matrix,
                elite_ratio=0.1,
                survivor_ratio=0.2,
                noise_intensity=10,
                sa_T=120.0,
        ...
        for _ in range(POP_SIZE):
            ...
            if ACO_ACTIVE:
                child = aco_guided_crossover(
                    p1, p2, NUM_CITIES, candidate_list, pheromone, ALPHA, BETA, distance_matrix
                )
            else:
        ...
        population = sorted(population + new_population,
                            key=lambda t: total_distance(t, distance_matrix, NUM_CITIES))[:POP_SIZE]
        best = population[0]
        best_score = total_distance(best, distance_matrix, NUM_CITIES)
        ...
        update_pheromones(
            population[:POP_SIZE //
                       4], NUM_CITIES, pheromone, EVAPORATION_RATE, Q, distance_matrix
        )
        ...
        # --- Algorithm used ---
        algo_used = "ACO" if ACO_ACTIVE else "SA" if SA_ACTIVE else "HBC" if HBC_ACTIVE else "GA pur"
